...
class EdwFilter:
        ...
        if len(class_codes) > 0:
            for idx, classCode in enumerate(class_codes):
                if "%" not in class_codes:
                    class_code_conditionals.append(
                        "V_JOB_DETL_HIST_1.JOB_DETL_EMPEE_CLS_CD LIKE :class_code_" + str(idx))
                else:
    ...
    def filter_college(self, college_code):
        college_conditional = "(V_JOB_DETL_HIST_1.JOB_DETL_COLL_CD = :coll_code OR " \
                              "V_EMPEE_HIST_1.EMPEE_COLL_CD = :coll_code)"
    ...
    def filter_exclude_netids(self, netids):
        ...
        if len(netids) > 0:
            for idx, netid in enumerate(netids):
                exclude_netid_conditionals.append(
                    "V_EMPEE_CAMPUS_EMAIL_ADDR.EMAIL_ADDR NOT LIKE :netid_exclude_" + str(idx))
